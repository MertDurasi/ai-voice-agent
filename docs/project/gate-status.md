...
| G2 Isolation | open | T-001–T-004; Cross-Tenant-Negativsuite und Rollenmatrix; `G0V` bestanden | – | [T-003 RLS-/Rollen-Nachweis](../tasks/tenancy/T-003-rls-db-roles.md); `T-004` offen |
...
- Engineering `Now`: `T-004` ergänzt die abgeschlossene PostgreSQL-RLS- und
  Rollenbaseline um inhaltsarme, unveränderliche Audit Events und schließt
  damit den noch offenen Engineering-Anteil von `G2` ab.
